...
local function inject_cookie()
    ...
    -- Baca XML existing dari Roblox (approach Getcookie.sh: baca dari shared_prefs asli)
    ...
    if existing ~= "" and existing:find("ROBLOSECURITY") then
        -- Preserve semua key lain, replace HANYA nilai .ROBLOSECURITY
        -- Escape % di cookie agar tidak diinterpretasi sebagai gsub pattern ref
        ...
        -- Fallback: fresh install / file belum ada
    ...
    -- Fix SELinux context supaya Roblox bisa baca file (penyebab utama inject gagal)
